...
def parse_google_timestr_to_24h_range(time_str):
    ...
    try:
        ...
        if '-' in t_clean:
            ...
            start_p_final = 'pm' if 'pm' in start_p else ('am' if 'am' in start_p else ('pm' if is_end_pm else 'am'))
...
def validate_meeting_time(meeting):
    ...
    try:
        ...
        start_dt = datetime.strptime(f"{meeting['start_date']} {meeting['start_time']}", "%d/%m/%Y %I:%M%p")
        end_dt = datetime.strptime(f"{meeting['end_date']} {meeting['end_time']}", "%d/%m/%Y %I:%M%p")
        ...
        if end_dt <= start_dt:
            ...
            if meeting['start_date'] == meeting['end_date']:
                return False, "End time is earlier than or equal to start time on the same day."
...
def parse_calendar_time(time_str):
    ...
    try:
        ...
        def to_24h(t_part, force_period=None):
            ...
            label = 'pm' if 'pm' in t_part.lower() else ('am' if 'am' in t_part.lower() else force_period)
        ...
        start_period = 'pm' if 'pm' in start_part.lower() else ('am' if 'am' in start_part.lower() else ('pm' if is_pm else 'am'))
...
def split_time_period(appointment):
    ...
    start_dt = datetime.strptime(f"{appointment['start_date']} {appointment['start_time']}","%d/%m/%Y %I:%M%p")
    end_dt = datetime.strptime(f"{appointment['end_date']} {appointment['end_time']}","%d/%m/%Y %I:%M%p")
...
def generate_conflict_message(conflict_data):
    convert_t_str = lambda t_str: datetime.strptime(t_str, "%H:%M").strftime("%I:%M%p").lower().lstrip('0')
    to_ampm = lambda time_tuple: f"{convert_t_str(time_tuple[0])} to {convert_t_str(time_tuple[1])}"
...
def add_calendar_event():
    with sync_playwright() as p:
        ...
        schedule_detail = {'meeting_name': 'Meeting with Mr. Trial', 'location': 'Lucky Restaurant', 'description': '', 'start_date': '31/12/2025', 'end_date': '31/12/2025', 'start_time': '07:00pm', 'end_time': '08:00pm'}

        print("Start filling information")
        page.fill('input[aria-label="Title"]', schedule_detail['meeting_name'])
        page.fill('input[aria-label="Start date"]', schedule_detail['start_date'])
        page.fill('input[aria-label="Start time"]', schedule_detail['start_time'])
        page.fill('input[aria-label="End date"]', schedule_detail['end_date'])
        page.fill('input[aria-label="End time"]', schedule_detail['end_time'])
        page.fill('input[aria-label="Add location"]', schedule_detail['location'])
        page.fill('div[aria-label="Description"]', schedule_detail['description'])
        page.get_by_label("Save").click()

        print("✅ Event added")
        context.close()

def get_event_from_date(dateStr) -> list[str]:
    """
    Check schedule conflict
    
    :param date: a string in MMDDYYYY
    """
    ...
    with sync_playwright() as p:
        context = p.chromium.launch_persistent_context('session', headless=False)
        page = context.new_page()
        page.goto('https://calendar.google.com')
        
        page.wait_for_selector('div[role="main"]')
        
        page.keyboard.press('a') # Use schedule view mode
        time.sleep(1)
        page.keyboard.press('g') # toggle "go to date"
        page.wait_for_selector('input[aria-label="Date"]')
        time.sleep(1)
        page.keyboard.type(formatted_date, delay=100)
        ...
        """
            In Google Calendar, for each day, each event account for a rows in the table. In each row, its second div contains the information of each event. Here we extract the event details by this pattern.
        """

        rows = day_container.locator('div[role="row"]').all()
        for row in rows:
            second_div = row.locator('xpath=./div[2]')
            if second_div.count() > 0:
                # Get and clean all text from all sub-divs
                nested_texts = second_div.locator('div').all_inner_texts()
...
def check_time_conflict(dates):
    required_key = ["start_date", "end_date", "start_time", "end_time"]
    if not all([i in dates for i in required_key]):
        return True

    init_prompt = f'I have trying to make a new scheduled meeting during dec 31 2025, from 10am-1pm. Here is the list of event during this date. If there are time conflict (holiday also count), list the time and the event name for all of the conflicted event in one single paragraph and ask the user to see if they want a different time, make the reply short: [[''All day'', ''Meeting with Tim'', ''tom abc, Accepted''], [''All day'', ""New Years Eve"", ''Calendar: Holidays in Canada, Accepted''], [''All day'', ''New Years Eve'', ''Calendar: Holidays in Hong Kong, Accepted''], [''10 – 10:30am'', ''Meeting'', ''tom abc, Accepted''], [''10:30am – 5:21pm'', ''Meeting'', ''tom abc, Accepted'']]'
...
def get_google_calendar_datekey(date_str):
    ...
    # The Google datekey calculation, from https://stackoverflow.com/questions/58080616/googlecalendar-datekey
    # Year since 1970 * 512 + month * 32 + day
